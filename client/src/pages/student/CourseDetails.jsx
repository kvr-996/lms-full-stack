    ...
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          ...
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Samar</span> </p>
                ...
                {courseData.courseContent.map((chapter,index)=>(
                  <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                    <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={()=> toggleSection(index)}>
                      ...
                      <p className='text-sm md:text-default text-gray-600'>{chapter.chapterContent.length} lectures-{calculateChapterTime(chapter)}</p>
                    </div>
                  <div className={`overflow-hidden transition-all duration-300 ${openSections[index]?'max-h-96':'max-h-0'}`}>{/*Voice said 3000*/}
                        ...
                        <li key={i} className='flex items-start gap-2 py-1'>
                          ...
                          <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'>
                            ...
                            <div className='flex gap-2'>
                              ...
                              <p>Total Duration:{humanizeDuration(lecture.lectureDuration*60*1000,{units:['h','m']})}</p>
                            </div>
                          </div>
                        </li>
                      ))}
        ...
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
          ...
          <button className='md:mt-6 mt-4 w-full py-3 
          rounded bg-blue-600
           text-white font-medium'>
            {isAlreadyEnrolled?'ALreadt Enrolled':'Enroll Now'}</button>
            <div className="pt-6">
